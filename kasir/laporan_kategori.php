...
try {
    ...
    |
    | PERHATIAN:
    | Jangan JOIN langsung ke barang_barcode karena satu barang_kemasan
    | bisa memiliki beberapa barcode.
    |
    | Untuk RETUR juga jangan JOIN langsung ke retur_penjualan_detail.
    | Kita agregasikan retur terlebih dahulu berdasarkan penjualan_detail_id.
    |
    | Dengan cara ini:
    |
    | Penjualan:
    ...
    |   modal_retur
    |
    | sehingga laporan kategori menjadi NETTO.
    |
    |--------------------------------------------------------------------------
    */
    ...
    $sqlDetail = "
        SELECT
             ...
             *
             * Modal penjualan:
             *   pd.qty * pd.harga_beli
             *
             * dikurangi modal barang yang diretur:
             *   qty_retur * harga_beli_retur
             */
            COALESCE(
                SUM(
        ...
        |--------------------------------------------------------------------------
        |
        | Satu penjualan_detail bisa memiliki beberapa retur.
        | Karena itu retur dikumpulkan dulu berdasarkan:
        |
        |   penjualan_detail_id
        |
        | Ini mencegah JOIN retur menggandakan hasil SUM().
        |
        */
        LEFT JOIN (
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Tetap menggunakan EXISTS supaya beberapa barcode untuk satu
    | barang_kemasan tidak menggandakan hasil SUM().
    |
    */
    ...
    */
    ...
    foreach ($rawDetail as $row) {
        ...
        */
        ...
        $rekapKategori[$katId]['total_qty']
            += $row['total_qty'];

        $rekapKategori[$katId]['total_omzet']
            += $row['total_omzet'];

        $rekapKategori[$katId]['total_modal']
            += $row['total_modal'];

        $rekapKategori[$katId]['total_keuntungan']
        ...
        <div class="col-md-3">
            ...
            <div class="card border-0 shadow-sm bg-primary text-white">
                ...
                <div class="card-body p-3">
                    ...
                    <h3 class="fw-bold mb-0 font-monospace">
                        Rp <?= number_format($grandOmzet, 0, ',', '.'); ?>
                    </h3>
            ...
            <?php else: ?>
                ...
                <div class="accordion accordion-flush" id="accordionKategori">
                    ...
                    <?php foreach ($rekapKategori as $katId => $kat): ?>
                        ...
                        $persenOmzet =
                            ($grandOmzet > 0)
                                ? ($kat['total_omzet'] / $grandOmzet) * 100
                                : 0;
                                    ...
                                    <div class="w-100 d-flex flex-wrap justify-content-between align-items-center me-3">
                                        ...
                                        <div class="d-flex align-items-center gap-3 mt-2 mt-md-0 font-monospace fs-6">
                                            ...
                                            <small class="text-muted fw-normal">
                                                ...
                                                <strong class="<?= $kat['total_keuntungan'] >= 0 ? 'text-success' : 'text-danger'; ?>">

                                                    Rp
                                                    <?= number_format(
                                                        $kat['total_keuntungan'],
                                                        0,
                                                        ',',
                                                        '.'
                                                    ); ?>
...
|--------------------------------------------------------------------------
|
| Tampilkan lengkap dengan Header & Footer.
|
*/
    ...
    <div class="row align-items-center mb-4">
        ...
        <div class="col-md-4">
            ...
            <p class="text-muted small mb-0">
                ...
                Analisis kontribusi omzet, modal, dan keuntungan bersih per kategori & barang.
