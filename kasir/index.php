...
// ============================================================
// CEK STATUS FITUR STOK (untuk tampilkan/sembunyikan badge stok)
// ============================================================
$cfgStok   = getPengaturanStok($pdoBarang);
$stokAktif = $cfgStok['fitur_stok']; // true / false

require_once BASE_PATH . 'partials/header.php';
?>

<!-- ============================================================ -->
<!-- CSS KHUSUS DROPDOWN PENCARIAN                                 -->
<!-- ============================================================ -->
<style>
...
/* Header dropdown (judul + tombol tutup) */
.search-dropdown .dropdown-header-custom {
  background: #f8f9fa;
...
.search-dropdown .dropdown-item-custom:last-child {
  border-bottom: none;
}

.search-dropdown .dropdown-item-custom:hover {
  background: #0d6efd;
  border-left-color: #0a58ca;
}

.search-dropdown .dropdown-item-custom:hover .item-nama,
.search-dropdown .dropdown-item-custom:hover .item-nama .text-muted {
  color: #ffffff !important;
}

.search-dropdown .dropdown-item-custom:hover .item-subinfo {
  color: #e0e0e0 !important;
}

.search-dropdown .dropdown-item-custom:hover .item-harga {
  color: #ffeb3b !important;  /* kuning cerah */
}

.search-dropdown .dropdown-item-custom:hover .item-stok {
  background: rgba(255, 255, 255, 0.2) !important;
  color: #ffffff !important;
  border-color: #ffffff !important;
}
.search-dropdown .dropdown-item-custom:active {
  background: #dbeaff;
}

/* Nama barang */
.search-dropdown .item-nama {
  font-weight: 600;
  color: #212529;
  white-space: nowrap;
  overflow: hidden;
...
/* Sub-info (kemasan • isi satuan) */
.search-dropdown .item-subinfo {
  font-size: 0.78rem;
  color: #6c757d;
  white-space: nowrap;
  overflow: hidden;
...
<!-- ============================================================ -->
<!-- DROPDOWN HASIL PENCARIAN (UPDATED — class search-dropdown)   -->
<!-- ============================================================ -->
<div id="searchResult"
...
<!-- ============================================================ -->
<!-- PASSING VARIABEL KE JS (untuk dropdown)                       -->
<!-- ============================================================ -->
<script>
window.STOK_AKTIF = <?= $stokAktif ? 'true' : 'false'; ?>;
</script>
<!-- Buka dengan tag <script> -->
<script>
document.addEventListener("DOMContentLoaded", function() {
  const tableFavorit = document.getElementById('tableFavorit');

  if (tableFavorit) {
    // 1 Listener untuk seluruh baris tabel
    tableFavorit.addEventListener('click', function(e) {
      // Cari elemen TR terdekat yang diklik
      const row = e.target.closest('.item-favorit-row');

      if (row && row.dataset.barang) {
        try {
          // Parse JSON dari atribut data-barang
          const itemData = JSON.parse(row.dataset.barang);

          // Panggil fungsi tambah ke keranjang
          addToCart(itemData);
        } catch (err) {
          console.error("Gagal parse data barang favorit:", err);
        }
      }
    });
  }
});
</script> <!-- Tutup tag </script> di sini -->
